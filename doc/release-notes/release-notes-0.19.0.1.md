...
- If a Tor hidden service is being used, Bitcoin Core will be bound to
  the standard port 8500 even if a different port is configured for
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  XTEcash), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in XTEcash per 1,000
...
- #16521 Use the default maxfeerate value as XTEcash/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as XTEcash/kB (jonatack)
